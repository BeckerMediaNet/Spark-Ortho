...
(function (window) {
  ...
  var BugDispatch = {
    ...
    add_events_to_bug: function add_events_to_bug(thebug) {
      var that = this;
      if (thebug.bug) {
    ...
    check_if_mouse_close_to_bug: function check_if_mouse_close_to_bug(e) {
      e = e || window.event;
      if (!e) {
        return;
      }
    ...
    near_bug: function near_bug(bug) {
      this.on_bug(bug);
    },
    on_bug: function on_bug(bug) {
      if (!bug.alive) {
        return;
      }
  ...
  var BugController = function BugController() {
    this.initialize.apply(this, arguments);
  };
  BugController.prototype = BugDispatch;
  var SpiderController = function SpiderController() {
    var spiderOptions = {
  ...
  var Bug = {
    ...
    initialize: function initialize(transform, options) {
      this.options = mergeOptions(this.options, options);
    ...
    setPos: function setPos(top, left) {
    ...
    moveBug: function moveBug(x, y, deg) {
    ...
    drawBug: function drawBug(top, left) {
      if (!this.bug) {
        this.makeBug();
      }
      if (!this.bug) return;
    ...
    toggleStationary: function toggleStationary() {
    ...
    fly: function fly(landingPosition) {
    ...
    startFlying: function startFlying(landingPosition) {
      var currentTop = this.bug.top,
        currentLeft = this.bug.left,
    ...
    dropping: function dropping(t, startPos, finalPos, rotationRate, deathType) {
    ...
    twitch: function twitch(deathType, legPos) {
      //this.bug.style.back
      if (!legPos) legPos = 0;
      var that = this;
  ...
  var SpawnBug = function SpawnBug() {
    var newBug = {},
      prop;
    for (prop in Bug) {
  ...
  var mergeOptions = function mergeOptions(obj1, obj2, clone) {
    if (typeof clone == 'undefined') {
      clone = true;
    }
    var newobj = clone ? _cloneOf(obj1) : obj1;
    for (var key in obj2) {
  ...
  var _cloneOf = function cloneOf(obj) {
    if (obj == null || _typeof(obj) != 'object') return obj;
    var temp = obj.constructor(); // changed

    for (var key in obj) {
      if (obj.hasOwnProperty(key)) {
        temp[key] = _cloneOf(obj[key]);
      }
    }
    return temp;
  };
...
/******/ 	__webpack_modules__["./src/js/bug.js"]();
/******/ 	
/******/ })()
;